...
/**
 * Replacement for plugin activated check
 *
 * @params    $plugin main plugin file path
 * @params    $callback main plugin function or class
 * @return    bool
 *
 * @access    public
 * @since     4.9.8.7
...
if ( !function_exists( 'ut_check_plugin_running' ) ) {
    function ut_check_plugin_running( $plugin, $callback = '' ) {
        if( ! empty( $callback ) && ( function_exists( $callback ) || class_exists( $callback ) ) ) {
            return true;
        }
        if( is_multisite() ) {
...
function ut_safe_svg( $svg ) {
    $svg_args = array(
        ...
        'symbol'=> array('id' => true, 'class'=> true),
        'text'  => array(
            'text-rendering' => true,
 ...
 * @since     4.9.8.7
 */
function ut_compare_values( $left, $right, $operation = '==' ) {
    switch ($operation){
        case '==':
            return $left == $right;
            break;
 ...
 * @since     4.9.8.7
 */
function ut_safe_meta_tag( $meta ) {
    return $meta;
}

function ut_safe_output( $out ) {
    return $out;
}
/**
 * Theme Icon
            ...
            <picture title="<?php echo wp_strip_all_tags( get_the_title( $post_id ) ); ?>">
                ...
                <?php if( !empty( $args['devices'] ) && is_array( $args['devices'] ) ) : ?>
                    ...
                    <?php foreach( $args['devices'] as $device ) :?>
                        ...
                        <source srcset="<?php echo esc_url( $image ); ?>, <?php echo esc_url( $retina ); ?> 2x" media="(<?php echo esc_attr($device[2]); ?>)">
...
if( !function_exists('ut_transform_button') ) {
    ...
    ob_start(); ?>
        ...
        <div id="<?php echo esc_attr( $wrap_id ); ?>" class="ut-hamburger-wrap">
            ...
            <a id="<?php echo esc_attr( $id ); ?>" class="ut-hamburger <?php echo esc_attr($style); ?>" type="button">
                <span></span>
            </a>
...
if( !function_exists('ut_hero_buttons') ) {
    ...
    function ut_hero_buttons() {
		...
		$ut_main_hero_button_attributes = implode(' ', array_map(
			function ($v, $k) { return sprintf("%s=\"%s\"", esc_attr($k), esc_attr($v)); },
        ...
        <span class="hero-btn-holder">
            ...
            <a id="to-about-section" target="<?php echo ut_return_hero_config( 'ut_main_hero_button_link_target' ); ?>" <?php echo filter_var($ut_main_hero_button_attributes); ?> href="<?php echo ut_compare_values($ut_main_hero_button_url_type, 'section') ? ut_clean_section_id( $ut_main_hero_button_target ) : $ut_main_hero_button_target; ?>" class="<?php echo implode(" ", $ut_main_hero_button_classes); ?> <?php echo esc_attr($ut_hero_animation_element_single); ?>">
            ...
            <?php if( $ut_second_hero_button == 'on' ) : ?>
                ...
                <a target="<?php echo ut_return_hero_config( 'ut_second_hero_button_target' ); ?>" <?php echo filter_var($ut_second_hero_button_attributes); ?> href="<?php echo ut_compare_values( $ut_second_hero_button_url_type, 'section' ) ? ut_clean_section_id( $ut_second_hero_button_url ) : $ut_second_hero_button_url; ?>" class="<?php echo implode(" ", $ut_second_hero_button_classes ); ?> <?php echo esc_attr($ut_hero_animation_element_single); ?>">
		...
		<div id="ut-custom-contact-section">
			...
			<div class="grid-container">
				...
				<div class="grid-100 mobile-grid-100 tablet-grid-100">
                    ...
                    if (ut_check_plugin_running( 'elementor/elementor.php' )) {

                        echo do_shortcode( $cblock->post_content );
...
if ( ! function_exists( 'ut_article_open_graph' ) ) : 
    ...
    function ut_article_open_graph() {
        ...
        echo ut_safe_meta_tag($open_graph);
...
if ( !function_exists( 'ut_get_preloader_font_settings' ) ) {
    ...
    function ut_get_preloader_font_settings() {
        ...
        if( ot_get_option( 'ut_image_loader_text_font_type' ) == 'ut-google' ) {
            ...
            $term_id = isset( $title_settings['font-family'] ) ? $title_settings['font-family'] : '';
            $font_term = ! empty( $term_id ) ? get_term( $term_id, 'unite_custom_fonts' ) : '';
            if( !empty( $font_term) ) {
                $title_settings['font-family'] = esc_attr( $font_term->name );
            }
...
function ut_social_icon_area_label( $icon ) {
    $icon = str_replace('fa-brands', '', $icon);
