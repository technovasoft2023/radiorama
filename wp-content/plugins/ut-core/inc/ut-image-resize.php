...
if( ! function_exists( 'ut_get_resized_quality' ) ) {
    function ut_get_resized_quality($retina = false)
    {
        $options = get_option('option_tree');
        $default = $retina ? 100 : 90;
        $option_name = $retina ? 'ut_image_quality_retina' : 'ut_image_quality';

        if (isset($options[$option_name]) && '' != $options[$option_name]) {
            return (int)$options[$option_name];
        }
...
if( !class_exists( 'UT_Resize' ) ) {
    ...
    class UT_Resize {
        ...
        public function get_image_path_from_url($image_url) {
        ...
        public function image_already_processed( $url, $width, $height ) {
            $image_path = $this->get_image_path_from_url( $url );

            if( ! isset( $image_path ) || ! isset( $width ) || ! isset( $height ) ) {
                return null;
            }
            $file_info = pathinfo($url);
            $ext = strtolower($file_info['extension']);
            $new_image = str_replace( '.'.$ext, '', $image_path );
            $new_image = $new_image.'-'.$width.'x'.$height.'.'.$ext;
            if( file_exists( $new_image ) ) {
                $new_url = str_replace( '.'.$ext, '', $url );
                return $new_url.'-'.$width.'x'.$height.'.'.$ext;
            }
        ...
        public function process( $url, $width = null, $height = null, $crop = null, $single = true, $upscale = false, $retina = true ) {
            if( function_exists('ot_get_option') && ot_get_option('ut_deactivate_fast_image_size', 'off' ) == 'on' ) {
                if ( $single ) {
                    $image = $url;
                } else {
            ...
            try {
                ...
                if ( ! $dims || ( ( ( null === $height && $orig_w == $width ) xor ( null === $width && $orig_h == $height ) ) xor ( $height == $orig_h && $width == $orig_w ) ) ) {
                    ...
                    // Else, we resize the image and return the new resized image url.
                        ...
                        $editor->set_quality( ut_get_resized_quality() );
                        ...
                        if ( $dst_x2_h && isset( $dst_rel_path ) && isset( $suffix ) ) {
                            ...
                            if ( file_exists( $destfilename ) && getimagesize( $destfilename ) && ! ut_compare_file_date( $img_path, $destfilename ) ) {
                                ...
                                if ( ! is_wp_error( $editor ) ) {
                                    ...
                                    $editor->set_quality( ut_get_resized_quality( true ) );
