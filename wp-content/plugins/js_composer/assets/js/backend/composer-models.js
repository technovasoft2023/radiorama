 ...
 *
 * WPBakery Page Builder backbone/underscore models for shortcodes.
 * ========================================================= */
...
(function ( $ ) {
	...
	vc.shortcode = Backbone.Model.extend( {
		...
		sync: function ( method, model, options ) {
			// optimize logic. when it is silent we should do nothing there.
	...
	vc.shortcodes_collection = Backbone.Collection.extend( {
		...
		_getShortcodeContent: function ( parent, state ) {
			var models, params;
			models = _.sortBy(
				vc.shortcodes.where( { parent_id: parent.get( 'id' ) } ),
				function ( model ) {
					return model.get( 'order' );
				}
			);
