 ...
 *
 * WPBakery Page Builder custom teaser block control meta box.
 * ========================================================= */
(function ( $ ) {
	'use strict';
	...
	var VCTeaser = Backbone.View.extend( {
		...
		renderButtons: function ( controls ) {
			_.each( controls, function ( button ) {
				var template = vc.template( $( '#vc_teaser-button' ).html(), vc.templateOptions.custom );
				var html = template( { label: window.i18nVcTeaser[ button + '_label' ], value: button } );
		...
		createControl: function ( data ) {
			...
			var template = vc.template( $( '#vc_teaser-' + data.name ).html(), vc.templateOptions.custom );
			var html = template( _.extend( default_data, data ) );
			this.$list.append( html );
			this.updateContent( data );
		},
		removeData: function ( data ) {
			if ( 'title' === data.name ) {
				$( '#title' ).off( 'keyup.vcTeaserTitle' );
			} else if ( 'text' === data.name ) {
		...
		updateContent: function ( data ) {
			if ( 'title' === data.name ) {
				$( '#title' ).on( 'keyup.vcTeaserTitle', this.updateTitle ).trigger( 'keyup.vcTeaserTitle' );
				this.save();
			} else if ( 'image' === data.name ) {
		...
		appendCustomImageHtml: function () {
			this.$spinner.hide();
			var template = vc.template( $( '#vc_teaser-custom-image' ).html(), vc.templateOptions.custom );
			$( '.vc_teaser-custom-image-view', this.$el ).html( template( this.custom_image_attributes ) );
		},
		setCustomImageBlock: function () {
			var $vc_image = $( '.vc_image', this.$el );
			var template = vc.template( $( '#vc_teaser-custom-image-block' ).html(), vc.templateOptions.custom );
			$vc_image.html( template() );
