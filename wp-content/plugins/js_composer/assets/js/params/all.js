 ...
 *
 * WPBakery Page Builder javascript functions to enable fields.
...
(function ( $ ) {
	...
	function InitGalleries() {
		$( '.gallery_widget_attached_images_list', this.$view ).off( 'click.removeImage' ).on( 'click.removeImage',
			'a.vc_icon-remove',
			function ( e ) {
				e.preventDefault();
		...
		$( '.gallery_widget_attached_images_list' ).each( function ( index ) {
			...
			$img_ul.sortable( {
				...
				update: function () {
					...
					$img_ul.closest( '.edit_form_line' ).find( '.gallery' + '' + '_widget_attached_images_ids' ).val( img_ids.join( ',' ) ).trigger( 'change' );
				}
			} );
		} );
	}
