...
(function ( $ ) {
	...
	vc.visualComposerView.prototype.openVcGitemEditForm = function ( e ) {
		...
		if ( vcGitemModel ) {
			vc.edit_element_block_view.render( vcGitemModel );
		}
	};
	...
	vc.visualComposerView.prototype.changeAnimation = function ( e ) {
		...
		if ( this.previewActivated ) {
			this.changePreviewAnimation( params.animation );
		}
	};
	vc.visualComposerView.prototype.changePreviewAnimation = function ( animation ) {
		var iframeWindow = $( '#vc_gitem-preview iframe' ).get( 0 ).contentWindow;
		if ( iframeWindow ) {
			iframeWindow.changeAnimation( animation );
		}
	};
	...
	vc.EditElementPanelView.prototype.ajaxData = function () {
		...
		if ( !_.isUndefined( params.content ) ) {
			merged_params.content = params.content;
		}
	...
	window.VcGitemView = window.VcColumnView.extend( {
		...
		setCZoneHeight: function () {
			...
			$left = $( '[data-vc-gitem="add-c"][data-vc-position="left"]' );
		...
		savePositionC: function ( event, ui ) {
			...
			cBlockContainer = ui.item.parent();
	...
	window.VcGitemAnimatedBlockView = window.VcColumnView.extend( {
		...
		addTab: function ( zone, id ) {
			var $zone = $( '<li class="vc_gitem-animated-block-tab vc_gitem-tab-' + zone + '"><a href="#" data-vc-gitem-tab-link="' + zone + '"><span class="vc_label">' + this.getZoneLabel( zone ) + '</span></a><a class="vc_control column_edit" data-vc-control="edit" data-vc-zone-model-id="' + id + '"><i class="vc-composer-icon vc-c-icon-mode_edit"></i></a></li>' );
			$zone.appendTo( '[data-vc-gitem-animated-block="tabs"]' );
			if ( 'a' === zone ) {
				this.switchZone( $zone.find( 'a' ) );
			}
		},
		...
		showEditForm: function ( e ) {
			...
			if ( model_id && vc.app.views[ model_id ] ) {
				vc.app.views[ model_id ].editElement();
			}
		},
		switchZone: function ( $zone ) {
			this.$el.find( '[data-vc-gitem-animated-block="tabs"] .vc_active, [data-vc-gitem-animated-block].vc_active' ).removeClass( 'vc_active' );
			$zone.parent().addClass( 'vc_active' );
			this.$el.find( '[data-vc-gitem-animated-block="add-' + $zone.data( 'vcGitemTabLink' ) + '"]' ).addClass( 'vc_active' );
		},
		setAEmpty: function () {
	...
	window.VcGitemZoneView = window.VcColumnView.extend( {
		...
		buildDesignHelpers: function () {
			...
			if ( image ) {
				$( '<span class="vc_css-helper vc_image-helper" data-vc-helper="image" style="background-image: url(' + image + ');" title="' + i18nLocale.column_background_image + '"></span>' )
					.insertBefore( $before );
			}
			if ( color ) {
				$( '<span class="vc_css-helper vc_color-helper" data-vc-helper="color" style="background-color: ' + color + '" title="' + i18nLocale.column_background_color + '"></span>' )
					.insertBefore( $before );
			}
	...
	window.VcGitemColView = window.VcColumnView.extend( {
		...
		changeTextAlign: function ( e ) {
			...
			if ( e ) {
				...
				this.$el.find( '> .vc_controls [data-vc-align="' + (this.model.getParam( 'align' ) || 'left') + '"]' ).addClass( 'vc_active' );
			}
		},
	...
	vc.AddElementUIPanelBackendEditor.prototype.createElement = function ( e ) {
		...
		var model, tag, parent_id = false, cZone, cZoneRow, cZoneCol, showSettings, shortcode;
