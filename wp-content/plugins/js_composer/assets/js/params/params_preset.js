			...
			if ( $option.length ) {
				...
				_.each( data, function ( value, key, list ) {
					...
					if ( $field.length ) {
						...
						if ( fieldManager && fieldManager.render ) {
							fieldManager.render( value );
						}
						fields.push( $field );
	...
	vc.atts.params_preset = {
		...
		init: function ( param, $field ) {
			...
			$( '.vc_params-preset-select', $field ).each( function () {
				var $this = $( this );
				if ( _.isUndefined( $this.data( 'fieldManager' ) ) ) {
					$this.data( 'fieldManager', new ParamsPresetView( { el: $this } ).render() );
				}
			} );
		}
	};
