	...
	window.vc.AiFormView = Backbone.View.extend( {
			...
			$.ajax( {
				...
				if ( true === response.success ) {
					...
					if ( response && response.data && response.data[0] && response.data[0].code && response.data[0].message ) {
						...
						console.error( window.i18nLocale.ai_response_error );
						_this.resetButton( false );
						_this.showErrorMessage( window.i18nLocale.ai_response_error );
					}
				}
		...
		changeContentType: function ( e ) {
			...
			if ( 'improve_existing' === e.target.value || 'translate' === e.target.value ) {
				this.$generate_button.text( window.i18nLocale.regenerate );
				...
				this.$generate_button.text( window.i18nLocale.generate );
		...
		insertContent: function () {
			...
			if ( 'textarea_html' === currentParamData.param_type ) {
				...
				window.tinymce.get( textareaId ).setContent( generatedContent );
				$textareaElement.val( generatedContent );
				...
				$inputElement.val( generatedContent );
			} else if ( currentParamData.fieldId ) {
				if ( aceEditorFields.includes( currentParamData.fieldId ) ) {
					var currentValue = window.ace.edit( currentParamData.fieldId ).getValue();
					generatedContent = currentValue + '\n\n' + generatedContent;
					window.ace.edit( currentParamData.fieldId ).setValue( generatedContent );
				} else if ( currentParamData.element.length ) {
					if ( 'new_content' === this.contentType ) {
