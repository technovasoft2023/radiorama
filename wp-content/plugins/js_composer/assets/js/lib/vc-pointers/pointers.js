...
(function ( $ ) {
	...
	vc.events.on( 'app.render', function () {
		...
		if ( window.vcPointer && window.vcPointer.pointers && window.vcPointer.pointers.length ) {
	...
	window.vcPointersEditorsTourEvents = function () {
		var $close_btn;
		$close_btn = this.pointer.domCloseBtn();
		$close_btn.bind( 'click.vcPointer', this.clickEventClose );
	...
	window.vcPointersShowOnContentElementControls = function () {
		if ( this.pointer && $( this.pointer.target ).length ) {
			...
			this.show();
			$( '#wpb_wpbakery' ).one( 'click', function () {
