...
<ul class="vc_general vc_ui-tabs-line" data-vc-ui-element="panel-tabs-controls">
	<?php foreach ( $tabs as $key => $v ) : ?>
		...
		?>
        <?php //@United ?>
        <li class="<?php echo esc_attr( implode( ' ', $classes ) ); ?> ut-<?php echo preg_replace('/\s+/', '', strtolower( $v['name'] ) );?>" data-vc-ui-element="panel-add-element-tab" <?php echo isset( $is_default_tab ) ? 'data-tab-index="' . esc_attr( $key ) .  '"' : '' ?>>
