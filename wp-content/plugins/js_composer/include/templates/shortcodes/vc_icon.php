...
if (class_exists("UT_Draw_SVG")) {
    // draw svg
    if( isset( $atts['color'] ) && $atts['color'] === 'custom' ) {
        $atts['svg_color'] = esc_attr( $atts['custom_color'] );
        ...
        $atts['svg_color'] = $colors[$atts['color']];
    }
    $svg = new UT_Draw_SVG(uniqid("ut-svg-"), $atts);
    $draw = $svg->draw_svg_icon();
}
...
$output .= ' vc_icon_element-size-' . esc_attr( $size ) . ' vc_icon_element-style-' . esc_attr( $background_style ) . ' vc_icon_element-background-color-' . esc_attr( $background_color ) . '" ' . $style . '><span class="vc_icon_element-icon ' . esc_attr( $iconClass ) . '" ' . ( 'custom' === $color ? 'style="color:' . esc_attr( $custom_color ) . ' !important"' : '' ) . '>'.$draw.'</span>';
