...
class Vc_Frontend_Editor extends Vc_Editor {
	...
	public function parseEditableContent( $post ) {
		...
		if ( $post_id > 0 && $post->ID === $post_id && ! defined( 'VC_LOADING_EDITABLE_CONTENT' ) ) {
			...
			ob_start();
			vc_include_template(
				'editors/partials/vc_welcome_block_front.tpl.php',
				[ 'editor' => 'frontend' ]
			);
	...
	public function renderEditButton( $link ) {
		if ( $this->showButton( get_the_ID() ) ) {
			return $link . ' <a href="' . esc_url( self::getInlineUrl() ) . '" id="vc_load-inline-editor" class="vc_inline-link">' . esc_html__( 'Edit with Brooklyn Website Builder', 'js_composer' ) . '</a>';
		}
	...
	public function renderRowAction( $actions ) {
		...
		if ( $this->showButton( $post->ID ) ) {
			$actions['edit_vc'] = '<a
		href="' . esc_url( $this->getInlineUrl( '', $post->ID ) ) . '">' . esc_html__( 'Website Builder', 'js_composer' ) . '</a>';
		}
		...
		if ( is_singular() ) {
			if ( $this->showButton( get_the_ID() ) ) {
				$wp_admin_bar->add_menu( array(
					...
					'title' => esc_html__( 'Edit with Brooklyn Website Builder', 'js_composer' ),
	...
	public function registerJs() {
        ...
        wp_register_script( 'pickr', vc_asset_url( 'lib/vendor/pickr/pickr.es5.min.js' ), array(), WPB_VC_VERSION, true );
        wp_register_script( 'vc-tooltipster', vc_asset_url( 'js/vendors/tooltipster.bundle.min.js' ), array(), WPB_VC_VERSION, true );
        wp_register_script( 'vc-tooltipster', vc_asset_url( 'js/vendors/tooltipster.bundle.min.js' ), array(), WPB_VC_VERSION, true );
        wp_register_script( 'vc-simplebar', vc_asset_url( 'js/vendors/simplebar.min.js' ), array(), WPB_VC_VERSION, true );
	...
	public function registerCss() {
        ...
        wp_register_style( 'pickr', vc_asset_url( 'lib/vendor/pickr/themes/classic.min.css' ), array(), WPB_VC_VERSION, false );
    }
