...
class Vc_Add_Element_Box {
    ...
    public function getControls() {
        $output = '<div class="vc-panel-no-results-message">' . __( '', 'js_composer' ) . '</div>';
    ...
    public function getFrontControls() {
        ...
        /** @var array $element */
        $buttons_count = 0;
        $cats = ['Plugins', 'Structural', 'Information', 'Community', 'Media', '_other_category_'];
        if( class_exists('WooCommerce') ) {
            $cats[] = 'WooCommerce';
        }
        foreach ( $cats as $cat ) {
            $codes = array_filter( $shortcodes, function ($shortcode) use ($cat) {
                return isset( $shortcode['category'] ) && $shortcode['category'] === $cat;
            } );
            if( $cat !== '_other_category_' ) {
                $output .= sprintf(
                    '<li data-element="ut-category-header" class="ut-layout-element-separator ut-vc-icon-module_nav ut-vc-icon-module_o ut-%s-module_nav vc_visible "><span>%s</span></li>',
                    esc_attr( strtolower($cat) ),
                    esc_html( $cat )
                );
            }
	...
	public function render() {
        if( isset( $_GET['vc_action'] ) && $_GET['vc_action'] === 'vc_inline' ) {
            vc_include_template( 'editors/popups/ut-add-element-front.tpl.php', array(
                'box' => $this,
                'editor'     => $this->editor,
