...
class Vc_Navbar_Frontend extends Vc_Navbar {
	...
	public function getControlScreenSize() {
		...
		if ( '1' !== $disable_responsive ) {
			...
			$output = '<li class="screens">' . '<div class="vc_dropdown" id="vc_screen-size-control">' . '<a href="#" class="vc_dropdown-toggle vc_icon-btn"' . ' title="' . esc_attr__( 'Responsive preview', 'js_composer' ) . '"><i class="vc-composer-icon vc_current-layout-icon vc-c-icon-layout_default"' . ' id="vc_screen-size-current"></i></a>' . '<ul class="vc_dropdown-list">';
	...
	public function getControlSaveUpdate() {
        ...
        <li class="vc_view">
            <a href="<?php echo get_permalink( $post ); ?>" target="_blank" title="<?php esc_html_e( 'View Page', 'js_composer' ); ?>"><i class="fa fa-eye"></i></a>
        </li>
		<?php
        echo $this->getControlScreenSize();
    ...
    public function getBottomControls() {
        $post = $this->post();
        $post_type = get_post_type_object( $this->post->post_type );
        $can_publish = current_user_can( $post_type->cap->publish_posts );
        ob_start();
            ...
            <li class="vc_show-mobile vc_nav-actions">
                ...
                if ( ! in_array( $post->post_status, array(
                    ...
                    <?php if ( 'draft' === $post->post_status ) {  ?>
                        ...
                        <li class="vc_nav-actions"><button type="button" class="vc_btn vc_btn-primary vc_navbar-btn vc_btn-save"
                    ...
                    <?php  } else { ?>
                        <button type="button" class="vc_btn vc_btn-primary vc_navbar-btn vc_btn-save"
                                id="vc_button-update" title="<?php esc_attr_e( 'Submit', 'js_composer' ); ?>"
                                data-change-status="pending"><?php esc_html_e( 'Submit', 'js_composer' ); ?></button>
                    <?php } ?>
    ...
    public function getControlToggleSidebar() {
        return '<li class="toggle-sidebar"><a href="#" title="'.esc_html__( 'Full Screen', 'js_composer' ).'" data-element="toggle-sidebar"><i class="fa-solid fa-up-right-and-down-left-from-center"></i></a></li>';
    }

    public function getControlSafeMode() {
        return '<li class="safe-mode"><a href="#" title="'.esc_html__( 'Safe Mode', 'js_composer' ).'" data-element="safe-mode"><i class="fa-solid fa-shield"></i></a></li>';
    }
}
