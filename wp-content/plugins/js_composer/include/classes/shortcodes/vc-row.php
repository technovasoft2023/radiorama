...
class WPBakeryShortCode_Vc_Row extends WPBakeryShortCode {
	...
	public function getLayoutsControl() {
		...
		$controls_layout .= '<a class="vc_control-set-column set_columns custom_columns" data-cells="custom" data-cells-mask="custom" title="' . esc_attr__( 'Custom layout', 'js_composer' ) . '">' . esc_html__( 'Custom', 'js_composer' ) . '</a> ';
	...
	public function getColumnControls( $controls, $extended_css = '' ) {
		...
		if ( is_array( $controls ) && ! empty( $controls ) ) {
			...
			if ( $allAccess ) {
				$row_edit_clone_delete .= '<span class="controls-ac ">' .$controls_clone. $controls_paste . $controls_copy .'</span>'. $controls_edit.$controls_toggle.$controls_delete;
			} elseif ( $editAccess ) {
				$row_edit_clone_delete .= $controls_edit.$controls_toggle;
			}
			$row_edit_clone_delete .= '</span>';

			if ( $allAccess ) {
				$output .= '<div class="controls-move">' . $controls_move . $controls_layout . $controls_add . '</div>' . $row_edit_clone_delete . $controls_end;
			} elseif ( $editAccess ) {
